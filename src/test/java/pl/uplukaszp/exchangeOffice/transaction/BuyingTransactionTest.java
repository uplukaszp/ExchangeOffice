...
public class BuyingTransactionTest {
	...
	public void isEnoughtMoneyTest() {
		...
		BuyingTransaction transaction = new BuyingTransaction(data, walletRepo, mainWalletRepo);
	...
	public void userHasEnoughtAndExchangeOfficeHasNotEnought() {
		TransactionData data = getTransactionData();
		data.setAmount(1000L);
		data.getMainWallet().setAmount(new BigDecimal(999));
		WalletRepository walletRepo = mock(WalletRepository.class);
		MainWalletRepository mainWalletRepo = mock(MainWalletRepository.class);
		BuyingTransaction transaction = new BuyingTransaction(data, walletRepo, mainWalletRepo);
	...
	public void verifyCalculationTest() {
		...
		assertTrue(transactionStatus);
		assertEquals(new BigDecimal(999), data.getUserSettlementWallet().getAmount());
		assertEquals(new BigDecimal(1001), data.getUserWallet().getAmount());
		assertEquals(new BigDecimal(1001), data.getMainSettlementWallet().getAmount());
		assertEquals(new BigDecimal(999), data.getMainWallet().getAmount());
